...
@end
@implementation BOBKeyboardUtil
- (void)dealloc {
...
#warning  2、如果textView获取光标时，光标位置不在可视范围内的话【被导航遮挡，或者溢出界面】，需要向下滚动到适合位置
    ...
    //MARGIN_KEYBOARD_DEFAULT 10 :编辑框底部高于键盘10像素
    if (CGRectGetMinY(keyboardRect) - MARGIN_KEYBOARD_DEFAULT < CGRectGetMaxY(convertRect)) {
        ...
        //updateOriginY 这里小于0
        CGFloat newOriginY = CGRectGetMinY(controllerView.frame) + signedDiff;//负数
        
        //3、如果大于键盘高度，上移会导致黑屏（uiwindow露出）
        if (fabs(newOriginY) > keyboardRect.size.height) {
            newOriginY = -keyboardRect.size.height;
            controllerView.y = -keyboardRect.size.height;
            CGFloat tableScrDatH = newOriginY + keyboardRect.size.height;
            
            if (self.tableViewArray.count > 0) {
                UITableView *firstTable = [self.tableViewArray firstObject];
                [firstTable setContentOffset:CGPointMake(0,firstTable.contentOffset.y - tableScrDatH) animated:NO];
            }
        } else {
            //4、没有大于键盘高度
            controllerView.y = newOriginY;
        }
        
    } else {
        if (convertRect.origin.y + controllerView.y < 64) {
            //情况1：父视图里有tableview（暂且考虑一级父视图）
            if (self.tableViewArray.count > 0) {
                UITableView *firstTable = [self.tableViewArray firstObject];
                [firstTable setContentOffset:CGPointMake(0,0) animated:NO];
            } else {
                //情况2：父视图没有tableview
                controllerView.y = 64 - convertRect.origin.y;
